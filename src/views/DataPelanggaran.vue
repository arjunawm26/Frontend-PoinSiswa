<template>
  <div>
              ...
              <div class="table-responsive">
                <b-table striped hover :items="pelanggaran" :fields="fields">
                  <!-- <template v-slot:cell(kategori)="data">
                    <b-badge variant="secondary">{{ data.item.kategori}}</b-badge>
                  </template> -->
    ...
    <b-modal id="modalTatib" @ok="Save">
      <template v-slot:modal-title>Form Pelanggaran</template>
      <form ref="form">
  ...
  module.exports = {
    ...
    methods: {
      getData: function() {
        ...
        this.axios
          .get("/pelanggaran/" + this.perPage + "/" + offset, conf)
          .then(response => {
      ...
      Save: function() {
        ...
        if(this.action === "insert") {
          ...
          this.axios
            .put("/pelanggaran/" + this.id, form, conf)
            .then(response => {
              this.$bvToast.hide("loadingToast");
              if (this.search == "") {
                this.getData();
